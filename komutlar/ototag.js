...
exports.run = async (client, message, args) => {
  ...
  if(miran !== "aç" && miran !== "kapat" && miran !== "kanal"){
      message.channel.send("Lütfen Bir Tag Belirt. **-ototag aç <TAGINIZ>**")
    return;
  }
  if(miran === "aç"){
   let codeming = args[1] 
    if(!codeming) return message.channel.send("Yeni Gelenlere Hangi Tagı Vereyim? Örnek Kullanım :**-ototag aç TEAM**")
